...
class QAGNN_Message_Passing(nn.Module):
    def __init__(self, args, k, n_ntype, n_etype, input_size, hidden_size, output_size,
        ...
        self.SAGPools = nn.ModuleList(
            [SAGPooling(hidden_size, ratio=self.pooling_ratio, GNN=GATConv, nonlinearity=torch.tanh) for _ in range(k)])
        ### 只用一层，相当于原来的k层共享了参数
    ...
    def mp_helper(self, _X, edge_index, edge_type, _node_type, _node_feature_extra,
        ...
        for i in range(self.k):
            ...
            _X, edge_index, edge_type, _node_type, batch, perm, score = self.SAGPools[i](_X, lm_node_scores, edge_index,
